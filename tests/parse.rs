...
Y98765432,ROTH IRA,FTIHX,FIDELITY INTL INDEX FUND,50.000,$42.10,+$0.12,$2105.00,+$6.00,+0.29%,-$100.00,-4.53%,10.0%,$2205.00,$44.10,Cash"#;
    let mut reader = Cursor::new(testdata);
    let portfolio = provider::load_portfolio(&mut reader, ProviderType::Fidelity)
...
#[test]
fn parse_vanguard() {
    let testdata = r#"Account Number,Investment Name,Symbol,Shares,Share Price,Total Value
...
87654321,Vanguard Federal Money Market Fund,VMFXX,500.000,1.00,500.00"#;
    let mut reader = Cursor::new(testdata);
    let portfolio = provider::load_portfolio(&mut reader, ProviderType::Vanguard)
